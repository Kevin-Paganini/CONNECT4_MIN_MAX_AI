...
class NeuralNetwork():
    ...
    def step(self, input):
        ...
        for i in range(len(self._W)):
            ...
            self._a[i+1] = self.g(np.dot(self._a[i], self._W[i]))    #sets next layer of inputs to the sigmoid of current layers outputs
    ...
    def mutate(self, rate, mu=0, s=1):
        ...
        for i in range(len(self._W)):                   #layer
            for j in range(len(self._W[i])):            #weight_group
                for k in range(len(self._W[i][j])):     #weight
                    if random() < rate:
                        self._W[i][j][k] += mu + random()*2*s - s   #FIXME might be wrong
    ...
    def get_weights(self):  #used for crossover
        return self._W
    def get_layers(self):
        return self._layers
